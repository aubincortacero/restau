              ...
              {restaurants.map((r) => {
                ...
                return (
                  <div key={r.id} className="flex items-center gap-1 px-2">
                    <form action={setActiveAction} className="flex-1 min-w-0">
                      <input type="hidden" name="id" value={r.id} />
                      <button
                        type="submit"
                        onClick={() => setOpen(false)}
                        className={`w-full flex items-center gap-2.5 px-2 py-2 rounded-lg text-sm transition-colors cursor-pointer text-left ${
                          isActive
                    ...
                    {/* Supprimer (seulement si pas le seul restaurant) */}
                    {restaurants.length > 1 && deleteRestaurantAction && (
                      confirmDelete === r.id ? (
                        <div className="flex gap-1 shrink-0">
                          <form action={deleteRestaurantAction}>
                            <input type="hidden" name="id" value={r.id} />
                            <button type="submit" className="text-[10px] text-red-400 hover:text-red-300 px-1.5 py-1 rounded cursor-pointer">
                              Oui
                            </button>
                          </form>
                          <button
