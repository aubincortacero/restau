...
function ChecklistSection({ items }: { items: ChecklistItem[] }) {
  ...
  return (
    <div className="border-t border-zinc-800 shrink-0">
      ...
      {/* Bouton relance du guide interactif */}
      <div className="px-4 pb-3 pt-1">
        <button
          onClick={() => window.dispatchEvent(new CustomEvent('tutorial:start'))}
          className="w-full flex items-center justify-center gap-1.5 text-[10px] text-zinc-600 hover:text-orange-400 transition-colors py-1.5 rounded-lg hover:bg-zinc-800/50 cursor-pointer"
        >
          <svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
