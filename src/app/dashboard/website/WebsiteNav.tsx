...
export default function WebsiteNav({
  ...
  // Détermine le label de la page courante pour mobile
  let currentLabel = 'Menu'
  if (appearanceActive) {
    currentLabel = 'Apparence'
  } else {
    const currentPage = pages.find(p => pathname.startsWith(`/dashboard/website/${p.id}`))
    if (currentPage) currentLabel = currentPage.title
  }
    ...
    <>
      ...
      <div className="md:hidden mb-6 w-full">
        ...
        >
          ...
          <svg
            className={`w-5 h-5 text-zinc-400 transition-transform ${mobileMenuOpen ? 'rotate-180' : ''}`}
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
            strokeWidth={2}
          ...
          <div className="mt-2 bg-zinc-900 border border-zinc-800 rounded-xl p-2 max-h-80 overflow-y-auto">
            ...
            {!adding ? (
              <button
                ...
                className="w-full flex items-center gap-2.5 px-3 py-2.5 rounded-lg text-sm text-zinc-600 hover:text-zinc-400 hover:bg-zinc-800/50 transition-colors border border-dashed border-zinc-800 hover:border-zinc-700 mt-2"
              >
              ...
              <div className="space-y-1.5 mt-2">
                ...
                <div className="flex gap-1">
                  ...
                  <button
                    ...
                    className="px-2.5 bg-zinc-800 hover:bg-zinc-700 text-zinc-400 text-xs rounded-lg transition-colors"
                  >
                    ✕
                  </button>
                </div>
